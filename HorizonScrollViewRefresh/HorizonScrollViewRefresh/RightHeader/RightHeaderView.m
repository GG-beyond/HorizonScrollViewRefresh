...
+ (instancetype)headerWithRefreshingTarget:(id)target refreshingAction:(SEL)action{
    ...
    RightHeaderView *cmp = [[self alloc] initWithFrame:CGRectZero];
...
- (void)layoutSubviews{
    ...
    _conImageView.center = CGPointMake(width/2.0, point.y + 30);
    _conText.center = CGPointMake(width/2.0, point.y - 20);
    _loadingView.center = _conImageView.center;
}
- (void)willMoveToSuperview:(UIView *)newSuperview{
    
    [super willMoveToSuperview:newSuperview];
    self.frame = CGRectMake(CGRectGetWidth(newSuperview.bounds), 0, 60, CGRectGetHeight(newSuperview.bounds));
}
- (void)prepare{
    
    self.stateTitles = @{
                         @(RefreshStateIdle):@"左拉可以刷新",
...
//    return;
    ...
    CGFloat rightWid = self.scrollView.contentSize.width;
    // 在刷新的refreshing状态
    if (self.state == RefreshStateRefreshing) {
        if (self.window == nil) return;
        
        [self.scrollView setContentInset:UIEdgeInsetsMake(0,rightWid+ 60, 0, 0)];
        return;
    }
    
    if (self.scrollView.isDragging) {//正在拖拽
        
        if (contentX>rightWid&&contentX<rightWid+60) {
            self.state = RefreshStateIdle;
        }else if (contentX>rightWid+60){
...
- (void)setState:(RefreshState)state{
    ...
    if (state == RefreshStateIdle) {
        ...
        [UIView animateWithDuration:0.25 animations:^{
            CGFloat top = 60;
            // 增加滚动区域top
            [self.scrollView setContentInset:UIEdgeInsetsMake(0, 0, 0, top)];
            // 设置滚动位置
            [self.scrollView setContentOffset:CGPointMake(self.scrollView.contentSize.width+top, 0) animated:NO];
        } completion:^(BOOL finished) {
        }];
    }
}
